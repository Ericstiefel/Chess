...
std::vector<Move> kingMoves(State& state, int attacker_ct) {
    ...
    for (int d : deltas) {
        ...
        if (to_sq_idx >= 0 && to_sq_idx < 64) {
            int file_diff = std::abs((from_sq_idx - to_sq_idx) % 8);
            if (file_diff <= 1 && !get_bit(own_occ, static_cast<uint8_t>(to_sq_idx))) {
